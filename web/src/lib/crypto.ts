...
/** Store the encrypted MEK envelope so it can be re-unwrapped after a page refresh.
 *  The envelope is already encrypted (wrapped with KEK derived from password) so
 *  localStorage is safe — the raw MEK stays in sessionStorage only. */
export function storeCryptoSession(mekEnvelope: string, mekSalt: string, argon2Params: string): void {
  localStorage.setItem(MEK_ENVELOPE_KEY, mekEnvelope);
  localStorage.setItem(MEK_SALT_KEY, mekSalt);
  localStorage.setItem(ARGON2_PARAMS_KEY, argon2Params);
}

export function getCryptoSession(): { mekEnvelope: string; mekSalt: string; argon2Params: string } | null {
  const mekEnvelope = localStorage.getItem(MEK_ENVELOPE_KEY);
  const mekSalt = localStorage.getItem(MEK_SALT_KEY);
  const argon2Params = localStorage.getItem(ARGON2_PARAMS_KEY);
...
export function clearCryptoSession(): void {
  sessionStorage.removeItem(MEK_KEY);
  localStorage.removeItem(MEK_ENVELOPE_KEY);
  localStorage.removeItem(MEK_SALT_KEY);
  localStorage.removeItem(ARGON2_PARAMS_KEY);
}
