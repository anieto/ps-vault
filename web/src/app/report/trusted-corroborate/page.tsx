...
import { CheckCircle2, AlertCircle, AlertTriangle, ShieldEllipsis } from "lucide-react";
import Image from "next/image";

function TrustedCorroborateContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await api.trustedCorroborate(token);
        setState("success");
      } catch (err) {
  ...
  const iconBg = state === "success" ? "bg-amber-50" : state === "error" ? "bg-red-100" : "bg-amber-50";
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-text-primary">
              {state === "loading" ? "Submitting…" : state === "success" ? "Report corroborated" : "Link unavailable"}
            </h1>
            <p className="text-xs text-text-muted">Trusted contact — corroborate report</p>
          </div>
        </div>
        ...
        {state === "success" && (
          <div className="space-y-2">
            <p className="text-sm text-text-secondary leading-relaxed">
              Your confirmation has been recorded. The vault owner has been notified and their response
              window has been shortened. Thank you for helping ensure their wishes are carried out.
            </p>
        ...
        {state === "error" && (
          <div className="space-y-2">
            ...
            <p className="text-sm text-text-muted">
              This can happen if the link has already been used, has expired, or the report is no longer active.
            </p>
          </div>
        )}
      </div>
...
export default function TrustedCorroboratePage() {
  return (
    <Suspense>
      <TrustedCorroborateContent />
    </Suspense>
  );
}
