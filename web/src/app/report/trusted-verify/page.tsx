...
function TrustedVerifyContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await api.trustedVerifyLife(token);
        setState("success");
      } catch (err) {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-text-primary">
              {state === "loading" ? "Confirming…" : state === "success" ? "Report dismissed" : "Link unavailable"}
            </h1>
            <p className="text-xs text-text-muted">Trusted contact — verify life</p>
          </div>
        </div>
        ...
        {state === "success" && (
          <div className="space-y-2">
            <p className="text-sm text-text-secondary leading-relaxed">
              The death report has been dismissed. The vault owner and the reporter have both been notified.
              The owner&apos;s check-in timer has been reset.
            </p>
...
export default function TrustedVerifyPage() {
  return (
    <Suspense>
      <TrustedVerifyContent />
    </Suspense>
  );
}
