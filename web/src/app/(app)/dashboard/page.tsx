...
export default function DashboardPage() {
  ...
  const { data: activeDeathReport } = useQuery({
    queryKey: ["death-report-active"],
    queryFn: () => api.getActiveDeathReport(),
      ...
      {activeDeathReport && (
        <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-200">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-amber-900">A death report has been filed</p>
            <p className="text-sm text-amber-700 mt-0.5">
              <strong>{activeDeathReport.reporter_name}</strong> has reported your passing.
              Checking in or clicking the &ldquo;I&rsquo;m Okay&rdquo; link in your email will clear this report.
              You must respond before{" "}
              <strong>{new Date(activeDeathReport.response_deadline).toLocaleString()}</strong>{" "}
              or your vaults will be released to your beneficiaries.
            </p>
          </div>
        </div>
      )}
...
function SwitchStatusCard({ sw }: { sw?: SwitchSettings }) {
  ...
  const loginCountsAsCheckin = branding?.login_counts_as_checkin !== "false";
  ...
  const checkInMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["death-report-active"] });
    },
    onError: (err) => {
    ...
    <Card className={isUrgent ? "border-amber-300 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-700/50" : "border-border"}>
      <CardContent className="flex items-start gap-3 py-4">
        ...
        <div className="flex-shrink-0 text-right space-y-1">
          ...
          {loginCountsAsCheckin && <p className="text-xs text-text-muted">Logging in also counts</p>}
        </div>
      </CardContent>
    </Card>
  );
