...
struct DashboardView: View {
    ...
    @State private var activeDeathReport: DeathReport? = nil
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    if let dr = activeDeathReport {
                        deathReportBanner(dr)
                    }
    ...
    // MARK: - Death report banner

    @ViewBuilder
    private func deathReportBanner(_ dr: DeathReport) -> some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text("A death report has been filed")
                    .font(.subheadline).fontWeight(.semibold)
                    .foregroundStyle(Color.orange)
                Text("\(dr.reporterName) has reported your passing. Checking in or tapping the I'm Okay link in your email will clear this report. You must respond before \(formatDeadline(dr.responseDeadline)) or your vaults will be released.")
                    .font(.caption)
    ...
    private func load() async {
        ...
        do {
            ...
            async let deathReportTask = APIService.shared.getActiveDeathReport()
            let (settings, loadedVaults, beneficiaries, deathReport) = try await (switchTask, vaultsTask, beneficiariesTask, deathReportTask)
            switchSettings = settings
            vaults = loadedVaults
            beneficiaryCount = beneficiaries.count
            activeDeathReport = deathReport
    ...
    var body: some View {
        HStack(alignment: .center, spacing: 12) {
            ...
            VStack(alignment: .trailing, spacing: 4) {
                ...
                if appState.loginCountsAsCheckin {
