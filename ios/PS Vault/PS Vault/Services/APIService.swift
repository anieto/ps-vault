...
final class APIService {
    ...
    struct BrandingResponse: Decodable {
        ...
        let loginCountsAsCheckin: String
        let registrationMode: String
        enum CodingKeys: String, CodingKey {
            case accentColor = "accent_color"
            case loginCountsAsCheckin = "login_counts_as_checkin"
    ...
    func submitAccessRequest(name: String, email: String, message: String) async throws {
        try await requestVoid("POST", path: "/access-request", body: [
    ...
    func getAdminConfig() async throws -> [String: String] {
        return try await request("GET", path: "/admin/config")
    }

    // MARK: - Death Report

    func getActiveDeathReport() async throws -> DeathReport? {
        return try await requestOptional("GET", path: "/report/active")
    }
}
