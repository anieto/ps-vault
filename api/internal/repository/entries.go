...
func (r *EntryRepo) BulkDelete(ctx context.Context, ids []string, vaultID string) error {
	if len(ids) == 0 {
		return nil
	}
	query, args, err := sqlx.In(`DELETE FROM vault_entries WHERE id IN (?) AND vault_id = ?`, ids, vaultID)
	if err != nil {
		return err
	}
