...
func (s *AdminService) DefaultRegistrationMode() string {
...
func (s *AdminService) SubmitAccessRequest(ctx context.Context, name, email, message string) error {
	...
	if admins, err := s.repos.Users.ListAdmins(ctx); err == nil {
		for _, admin := range admins {
			s.email.SendAsync(ctx, admin.Email, "access_request", data)
		}
	}
	return nil
}
